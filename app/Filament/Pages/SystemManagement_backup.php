...
class SystemManagement extends Page implements HasTable
{
    ...
    public function table(Table $table): Table
    {
        return $table
            ->query(
                Order::query()
                    ->with(['user', 'customer', 'items', 'paymentLogs'])
                    ->latest()
            )
            ->columns([
                ...
                Tables\Columns\TextColumn::make('user_info')
                    ...
                    ->searchable(query: function (Builder $query, string $search) {
                        $query->where(function (Builder $subQuery) use ($search) {
                            $subQuery->whereHas('user', function (Builder $userQuery) use ($search) {
                    ...
                    ->formatStateUsing(function ($record) {
                        ...
                        if ($record->user_id && $record->user) {
                            return $record->user->name ?? '用戶 #' . $record->user_id;
                        }

                        // 對於一般店家訂單，顯示顧客名稱
                        if ($record->customer_id && $record->customer) {
                            return $record->customer->name ?? '顧客 #' . $record->customer_id;
                        }
                        ...
                        // 顯示ID信息作為後備
                        if ($record->user_id) {
                            return '訂閱用戶 #' . $record->user_id;
                        }

                        if ($record->customer_id) {
                            return '顧客 #' . $record->customer_id;
                        }
            ...
            ->actions([
                ...
                Actions\Action::make('view_details')
                    ...
                    ->modalContent(fn (Order $record) =>
                        view('filament.modals.order-details', ['order' => $record->load(['user', 'customer', 'items', 'paymentLogs'])])
                    ),
